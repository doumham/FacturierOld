...
?>
      ...
      <table> 
        <tr class="legende">
          ...
          <th class="aR">n°</th>
          <th><?php if($ordre){?><a href="entrees.php?annee=<?php echo $annee?>" title="Ordonner par dates">Date</a><?php }else{?>Date ↓<?php }?></th>
          <th><?php if(!$ordre){?><a href="entrees.php?annee=<?php echo $annee?>&amp;ordre=clients" title="Ordonner par clients">Client<?php }else{?>Client ↓<?php }?></a></th>
          <th>TVA</th>
...
<?php if (substr($f['date'],0,4) < $date_limite): ?>
    ...
    <div class="contenu">
      <h3><a href="entrees.php?annee=<?php echo substr($f['date']+1,0,4) ?>"><?php echo substr($f['date']+1,0,4) ?></a></h3>
      <table>
<?php endif ?>
<?php
