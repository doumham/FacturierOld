...
?>
      <h3><a href="depenses.php?annee=<?php echo $annee_h3 ?>"><?php echo $annee_h3 ?></a></h3>
    <table> 
      <tr class="legende">
...
<?php }?>
  <tr <?php if ($f['deductibilite']==0){echo "style='background-image:url(images/fond_li_unpaid.png)'";}?> class="facture" id="element_<?php echo $f['id']?>">
    <td>
      <a id="del_paid_<?php echo $f['id']?>" href="#" onclick="effacer_depense(<?php echo "'".$f['id']."'";?>);return false;" title="Supprimer">
        <img id="icn_del_<?php echo $f['id']?>" src="images/icn-delete.png" alt="Supprimer" />
      </a>
      <a href="form_depense.php?annee=<?php echo $annee ?>&amp;id=<?php echo $f['id']?>" title="Modifier">
...
<?php if (substr($f['date'],0,4) < $date_limite): ?>
    ...
    <div class="contenu">
      <h3><a href="depenses.php?annee=<?php echo substr($f['date']+1,0,4) ?>"><?php echo substr($f['date']+1,0,4) ?></a></h3>
      <table>
    <?php endif ?>
<?php
