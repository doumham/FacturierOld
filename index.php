...
$select_factures = mysql_query("
	SELECT * FROM factures LEFT JOIN clients ON factures.id_client=clients.id_client ".$req." ORDER BY ".$req2." date, numero
	") or trigger_error(mysql_error(),E_USER_ERROR);?>
<?php $nombre_factures = mysql_num_rows($select_factures); ?>
...
?>
      <h3><a href="index.php?annee=<?php echo $annee_h3 ?>"><?php echo $annee_h3 ?></a></h3>
      <table> 
        <tr class="legende">
          <th><span class="no_print">Outils</span></th>
          <th class="aR">n°</th>
          <th><?php if($ordre){?><a href="index.php?annee=<?php echo $annee?>" title="Ordonner par dates">Date</a><?php }else{?>Date ↓<?php }?></th>
          <th><?php if(!$ordre){?><a href="index.php?annee=<?php echo $annee?>&amp;ordre=clients" title="Ordonner par clients">Client<?php }else{?>Client ↓<?php }?></a></th>
          <th>TVA</th>
...
<?php }?>
  <tr <?php if ($f['paid'] == 0){echo "style='background-image:url(images/fond_li_unpaid.png)'";}?> class="facture" id="element_<?php echo $f['id']?>">
    <td>
      <a id="del_paid_<?php echo $f['id']?>" href="#" onclick="effacer(<?php echo "'".$f['numero']."','".$f['id']."','".$f['paid']."'";?>);return false;" title="Supprimer">
        <img id="icn_del_<?php echo $f['id']?>" src="<?php if ($f['paid'] == 0) {
        	echo "images/icn-delete.png";
        } else {
        	echo "images/icn-delete-off.png";
        }
         ?>" alt="Supprimer" />
      </a>
      <a href="form_facture.php?annee=<?php echo $annee ?>&amp;id=<?php echo $f['id']?>" title="Modifier">
...
<?php if (($key_trimestre == "4" && !$ordre)||($_GET['annee'] == date("Y") && $count_facture == $nombre_factures)){ ?>
        <tr class="tot_annee">
...
<?php if (substr($f['date'],0,4) < $date_limite): ?>
    ...
    <div class="contenu">
      <h3><a href="index.php?annee=<?php echo substr($f['date']+1,0,4) ?>"><?php echo substr($f['date']+1,0,4) ?></a></h3>
      <table>
<?php endif ?>
<?php
